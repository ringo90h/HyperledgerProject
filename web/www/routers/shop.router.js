...
router.post('/api/contract-types', async (req, res) => {
    ...
    let contractTypes;
    try {
        contractTypes = await Peer.getContractTypes(letter);
    } catch (e) {
        console.log(e);
...
router.post('/api/request-user', async (req, res) => {
    ...
    if (typeof user === 'object' &&
        ...
        try {
            let responseUser = await Peer.createUser({
                username: email,
                firstName: firstName,
                lastName: lastName,
...
router.post('/api/enter-contract', async (req, res) => {
    ...
    if (typeof user === 'object' &&
        ...
        try {
            ...
            let loginInfo = await Peer.createContract({
                contractTypeUuid,
                username,
                password: passwordProposal,
                firstName,
...
router.post('/api/blocks', async (req, res) => {
    ...
    try {
        const blocks = await Peer.getBlocks(noOfLastBlocks);
        res.json(blocks);
    } catch (e) {
